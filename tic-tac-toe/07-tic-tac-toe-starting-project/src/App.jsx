...
function deriveWinner(gameBoard, playerName){
  ...
  for(const combo of WINNING_COMBINATIONS){
    ...
    if(firstSq && firstSq == secondSq && secondSq == thirdSq) winner = playerName[firstSq]
  }
  return winner
}
...
//GameBoard and Player both components/ states have access to some info., to highlight the turn of each player. So, we have to lift the state up, means storing info. in the closest ancestor, which is App.jsx here, do that it has access to all the components required to work with that state
  ...
  //Currently managing activePlayer as a state, as have to update UI, the box has to swiych and so do symbols. But this can be done using gameTurns state, so no need of managing as a state, just to trigger UI. Instead, using a helper function
  ...
  function sqHandler(rowIdx, colIdx){
    setGameTurns(prevTurns => {
      ...
      const updatedTurns = [{ sq: { row: rowIdx, col: colIdx }, player: currentPlayer}, ...prevTurns]
      return updatedTurns
    })
  }
    ...
    <main>
      <div id="game-container">
        <ol id="players" className="highlight-player">
          <Player name={PLAYERS.X} symbol="X" isActive={activePlayer === 'X'} onNameChange={playerNameHandler}/>
          <Player name={PLAYERS.O} symbol="O" isActive={activePlayer === 'O'} onNameChange={playerNameHandler}/>
        </ol>
